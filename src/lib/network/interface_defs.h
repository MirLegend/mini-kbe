...
namespace Network
{
#define MSGIDMAKE(main,sub) ((uint16)main<<8 | sub)
	...
	#define NETWORK_MESSAGE_HANDLER(DOMAIN, MSGID, NAME, HANDLER_TYPE, MSG_LENGTH, ARG_N)						\
		HANDLER_TYPE* p##NAME = static_cast<HANDLER_TYPE*>(messageHandlers.add(MSGID, #DOMAIN"::"#NAME,		\
...
#define NETWORK_MESSAGE_DECLARE_STREAM(DOMAIN, MSGID, NAME, MSGHANDLER,	\
											MSG_LENGTH)				\
	NETWORK_MESSAGE_HANDLER(DOMAIN, MSGID, NAME, MSGHANDLER, MSG_LENGTH, 	\
														_stream)	\
	MESSAGE_STREAM(NAME)											\

	...
	while(pPacket->length() > 0 || pFragmentStream_ != NULL)
	{
		if(fragmentDatasFlag_ == FRAGMENT_DATA_UNKNOW)
		{
			...
			if(currMainCmd == 0)
			{
				...
				(*pPacket) >> currMainCmd;
				(*pPacket) >> currSubCmd;
				pPacket->messageID(MSGIDMAKE(currMainCmd, currSubCmd));
			}
			...
			if(pMsgHandler == NULL)
			{
				...
				ERROR_MSG(fmt::format("PacketReader::processMessages: not found msgID={}/{}, from {}.\n",
					(uint32)currMainCmd, (uint32)currSubCmd, pChannel_->c_str()));
			...
			else
			{
				...
				pPacket->rpos(pPacket->rpos() - NETWORK_MESSAGE_LENGTH1_SIZE);
				// 临时设置有效读取位， 防止接口中溢出操作
				size_t wpos = pPacket->wpos();
				// size_t rpos = pPacket->rpos();
				size_t frpos = pPacket->rpos() + currMsgLen_ + NETWORK_MESSAGE_LENGTH1_SIZE;
				pPacket->wpos(frpos);
...
void PacketReader::mergeFragmentMessage(Packet* pPacket)
{
	...
	if(pPacket->length() >= pFragmentDatasRemain_)
	{
		...
		case FRAGMENT_DATA_MESSAGE_ID:			// 消息ID信息不全
			memcpy(&currMainCmd, pFragmentDatas_, sizeof(uint8));
			memcpy(&currSubCmd, pFragmentDatas_, sizeof(uint8));
			break;

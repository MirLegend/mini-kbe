...
public:
	...
	Network::MessageID	currMsgID() const{return MSGIDMAKE(currMainCmd, currSubCmd);}
	Network::MessageLength	currMsgLen() const{return currMsgLen_;}
	
	void currMsgID(uint8 _mainCmd, uint8 _subCmd) { currMainCmd = _mainCmd; currSubCmd = _subCmd; }
